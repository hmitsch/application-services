...
impl Login {
    ...
    pub fn from_row(row: &Row) -> Result<Login> {
        Ok(Login {
            ...
            time_last_used: row.get_checked("timeLastUsed")?,
...
impl SyncLoginData {
    ...
    pub fn from_payload(payload: sync::Payload, ts: ServerTimestamp) -> Result<Self> {
        ...
        let login: Option<Login> = if payload.is_tombstone() { None }
                                   else { Some(payload.into_record()?) };
...
impl Login {
    ...
    pub fn apply_delta(&mut self, mut delta: LoginDelta) {
    ...
    pub fn delta(&self, older: &Login) -> LoginDelta {
