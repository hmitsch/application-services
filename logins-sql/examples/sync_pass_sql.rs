...
fn timestamp_to_string(microsecs: i64) -> String {
...
fn show_sql(e: &PasswordEngine, sql: &str) -> Result<()> {
    ...
    let mut stmt = e.db.db.prepare(sql)?;
...
fn show_all(engine: &PasswordEngine) -> Result<Vec<String>> {
    ...
    let last_sync = engine.db.get_last_sync()?.unwrap_or_default();
    let changes = engine.db.fetch_outgoing(last_sync)?;
    info!("Engine has {} records, a last sync timestamp of {}, and {} queued changes",
           records.len(), last_sync, changes.changes.len());
